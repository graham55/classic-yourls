...
Purpose:
--------
This file documents additional shortcode functionality and settings that I developed on top of the existing Classic YOURLS plugin. It is *not* an official fork, but a contribution intended for consideration by the original plugin author.
...
Added Functionality
--------------------
- Introduced a new WordPress shortcode: [classic_yourls_shortlink]
- Example usages:
  [classic_yourls_shortlink id="123"]
  [classic_yourls_shortlink id="123" text="Click here"]
  [classic_yourls_shortlink] (defaults to current post ID)
...
- Below the existing input field, the metabox now displays:
  ...
  - A usage example of the shortcode for that post:
    [classic_yourls_shortlink id="{post_id}"]
